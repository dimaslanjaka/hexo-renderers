...
const anchorId = (strOrObject: string | marked.Tokens.Heading, transformOption: number) => {
...
function mangleEmail(text: string) {
  let out = '';
...
export class MarkedRenderer extends marked.Renderer {
  ...
  heading({ text, depth }: marked.Tokens.Heading): string {
    const { anchorAlias, headerIds, modifyAnchors, _headingId } = this.options as CustomMarkedOptions;
  ...
  link({ href, title, text }: marked.Tokens.Link): string {
    const { external_link, sanitizeUrl, hexo, mangle } = this.options as CustomMarkedOptions;
  ...
  paragraph({ text }: marked.Tokens.Paragraph): string {
    const { descriptionLists = true } = this.options as CustomMarkedOptions;
  ...
  image({ href, title, text }: marked.Tokens.Image): string {
    const options = this.options as CustomMarkedOptions;
    const { hexo } = options;
    ...
    if (!/^(#|\/\/|http(s)?:)/.test(href) && !relative_link && prependRoot) {
      if (!href.startsWith('/') && !href.startsWith('\\') && postPath) {
        ...
        const asset = PostAsset.findById(upath.join(postPath, href.replace(/\\/g, '/')));
