...
export function getRelatedPosts(hexo) {
    const options = hexo.config.renderers?.generator;
    ...
    hexo.extend.helper.register('list_related_posts', function (options) {
        ...
        const relatedDb = path.join(hexo.base_dir, 'tmp/hexo-renderers/related-posts', slugify(this.page?.title), 'related.json');
        options = assign({
            maxCount: 5,
            orderBy: 'date',
            isAscending: false,
        ...
        if (postList.length > 0) {
            ...
            */
            const currentPostIndex = postList.findIndex((post) => post._id === this.page?._id || post.title === this.page?.title);
