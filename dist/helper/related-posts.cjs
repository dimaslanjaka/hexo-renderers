...
var collector = require('./collector.cjs');
...
function list_related_posts(options) {
    ...
    const relatedDb = path.join(hexo.base_dir, 'tmp/hexo-renderers/related-posts', sbgUtility.slugify((_a = this.page) === null || _a === void 0 ? void 0 : _a.title), 'related.json');
    options = assign({
        maxCount: 5,
        orderBy: 'date',
        isAscending: false,
    ...
    if (fs.existsSync(relatedDb)) {
        // load from cache
        postList = sbgUtility.jsonParseWithCircularRefs(fs.readFileSync(relatedDb, 'utf-8'));
    }
    else {
        // regenerate cache
        ...
        if (postList.length === 0) {
            ...
            const postData = collector.getPostData().filter((post) => {
                var _a;
                let tags = [];
    ...
    if (postList.length > 0) {
        ...
        */
        ...
        if (options.orderBy === 'random') {
            sbgUtility.array_shuffle(postList);
        }
        else {
            postList.sort(dynamicSort(options.orderBy, options.isAscending));
        }
        postList.sort(dynamicSort('count', false));
        sbgUtility.writefile(relatedDb, sbgUtility.jsonStringifyWithCircularRefs(postList));
    }
    return postList;
}
/**
...
function getRelatedPosts(hexo) {
    ...
    hexo.extend.helper.register('list_related_posts', list_related_posts);
}
